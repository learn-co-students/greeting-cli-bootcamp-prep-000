...
describe './bin/greet executing a CLI Application' do
  it 'outputs a welcome message and asks the user for input' do
    ...
    expect($stdout).to receive(:puts).with(rspec ./spec/02_cli_spec.rb:31 # ./bin/greet executing a CLI Application calls on the #greeting method with an argument of the user's name and returns the new greeting, interpolatin
g the user's name), "Make sure bin/greet has code that can output 'Hi! I'm HAL, what's your name?' exactly."
